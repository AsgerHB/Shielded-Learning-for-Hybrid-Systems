...
/* formula 1 */
strategy DeathCosts1000 = minE (LearnerPlayer.fired + (number_deaths > 0)*1000 ) [<=120] {} -> {p, v}: <> time >= 120
...
/* formula 6 */
strategy DeathCosts100 = minE (LearnerPlayer.fired + (number_deaths > 0)*100 ) [<=120] {} -> {p, v}: <> time >= 120
...
/* formula 11 */
strategy DeathCosts10 = minE (LearnerPlayer.fired + (number_deaths > 0)*10 ) [<=120] {} -> {p, v}: <> time >= 120
