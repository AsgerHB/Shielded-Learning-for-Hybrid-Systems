...
"""
    compile_libmb2shield(working_dir, shield_dir, lib_source_code_dir)

Load shield from `shield_dir`, and bake it into a `libmb2shield.so` file.
...
"""
function compile_libmb2shield(working_dir, shield_dir, lib_source_code_dir)
    ...
    write(working_dir ⨝ "shield_dump.c", get_c_library_header(shield, "Retrieved with get_libmb2shield"))'
    ...
    run(`gcc -shared -o libmb2shield.so shield.o`)

    cd(previous_working_dir)
    working_dir ⨝ "libmb2shield.so"
end

function get_libmb2shield(possible_shield_file, lib_source_code_dir, lib_destination_dir; working_dir, test=false)
    
    # Getting shield
    shield_dir = get_shield(possible_shield_file, working_dir; test)

    lib_destination_dirʹ = compile_libmb2shield(working_dir, shield_dir, lib_source_code_dir)
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    test = true
    lib_destination_dir = "/home/asger/libmb2shield.2.so"
    lib_source_code_dir = "/home/asger/Insync/OQ82YK@cs.aau.dk/OneDrive Biz - SharePoint/DEIS - Shielded AI - TACAS Paper - Dokumenter/Shielded AI - TACAS Paper/Code/Shared Code/libmb2shield"
    println("Running as standalone script. This is suitable for testing.")
    result = get_libmb2shield(possible_shield_file, lib_source_code_dir, lib_destination_dir; test)
