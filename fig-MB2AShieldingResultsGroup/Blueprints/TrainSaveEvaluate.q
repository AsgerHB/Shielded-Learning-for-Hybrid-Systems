...
/* formula 1 */
strategy Deterrence1000 = minE (LearnerPlayer.fired + (number_deaths > 0)*1000 ) [<=120] {} -> {p[0], v[0], p[1], v[1]}: <> time >= 120
...
/* formula 6 */
strategy Deterrence100 = minE (LearnerPlayer.fired + (number_deaths > 0)*100 ) [<=120] {} -> {p[0], v[0], p[1], v[1]}: <> time >= 120
...
/* formula 11 */
strategy Deterrence10 = minE (LearnerPlayer.fired + (number_deaths > 0)*10 ) [<=120] {} -> {p[0], v[0], p[1], v[1]}: <> time >= 120
...
/* formula 16 */
strategy Deterrence0 = minE (LearnerPlayer.fired + (number_deaths > 0)*0 ) [<=120] {} -> {p[0], v[0], p[1], v[1]}: <> time >= 120
